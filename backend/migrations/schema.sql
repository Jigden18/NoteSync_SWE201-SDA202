...
CREATE UNIQUE INDEX IF NOT EXISTS "modules_enrol_code_key" ON "modules"("enrol_code");
...
-- Full-text search trigger and GIN index on note_versions
ALTER TABLE "note_versions" ADD COLUMN IF NOT EXISTS "tsv" tsvector;
CREATE INDEX IF NOT EXISTS "note_versions_tsv_idx" ON "note_versions" USING gin("tsv");

CREATE OR REPLACE FUNCTION note_versions_tsv_trigger() RETURNS trigger AS $$
BEGIN
...
DROP TRIGGER IF EXISTS tsvectorupdate ON note_versions;
CREATE TRIGGER tsvectorupdate BEFORE INSERT OR UPDATE
ON note_versions FOR EACH ROW EXECUTE FUNCTION note_versions_tsv_trigger();
