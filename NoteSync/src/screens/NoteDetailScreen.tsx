  ...
  return (
    <ScrollView style={styles.tabContent} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.contentContainer}>
        {user?.role === "student" && !note.isLocked && (
          <TouchableOpacity
            onPress={() => {
              setSelectionMode(!selectionMode);
              setSelectedText("");
            }}
            style={styles.selectionModeToggle}
          >
            <Ionicons name={selectionMode ? "eye-outline" : "create-outline"} size={16} color={C.accent} style={{ marginRight: 6 }} />
            <Text style={styles.selectionModeToggleText}>
              {selectionMode ? "Switch to Reader View" : "Suggest Inline Edit (Selection Mode)"}
            </Text>
          </TouchableOpacity>
        )}
          ...
          filtered.map((p) => {
            ...
            return (
              <View key={p.id} style={styles.proposalCard}>
                ...
                {!p.isInline && expanded[p.id] && (
                  <View style={styles.fullDiffContainer}>
                    <Text style={styles.fullDiffLabel}>Current note</Text>
                    <Text style={styles.fullDiffRemoved}>{p.originalText || "Original note content unavailable"}</Text>
                    <Text style={styles.fullDiffLabel}>Proposed note</Text>
                    <Text style={styles.fullDiffAdded}>{p.suggestedText || "Proposed note content unavailable"}</Text>
                  </View>
                )}
...
const styles = StyleSheet.create({
  ...
  selectionModeToggle: { flexDirection: "row", alignItems: "center", justifyContent: "center", backgroundColor: C.accentLight, paddingVertical: 8, paddingHorizontal: 12, borderRadius: 8, marginBottom: 12, borderWidth: 1, borderColor: C.accent },
  selectionModeToggleText: { color: C.accent, fontSize: 13, fontWeight: "600" },
  previewBox: { backgroundColor: C.surface, borderRadius: 12, borderWidth: 1, borderColor: C.border, padding: 16, minHeight: 300 },
  h1: { fontSize: 24, fontWeight: "700", color: C.textPrimary, marginVertical: 10, marginTop: 16 },
  h2: { fontSize: 20, fontWeight: "600", color: C.textPrimary, marginVertical: 8, marginTop: 12 },
  ...
  listItem: { flexDirection: "row", marginBottom: 6, alignItems: "flex-start", paddingLeft: 8 },
  ...
  fullDiffContainer: { backgroundColor: C.bg, borderRadius: 8, padding: 10, borderWidth: 1, borderColor: C.border, gap: 10, marginBottom: 8 },
  fullDiffLabel: { fontSize: 12, fontWeight: "700", color: C.textSecondary },
  fullDiffRemoved: { color: C.danger, fontSize: 13, lineHeight: 20 },
  fullDiffAdded: { color: C.success, fontSize: 13, lineHeight: 20 },
});
