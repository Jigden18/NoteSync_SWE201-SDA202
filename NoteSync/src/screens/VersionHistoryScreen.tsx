...
export const VersionHistoryScreen: React.FC<VersionHistoryScreenProps> = ({
  ...
  const { getVersionsForNote } = useNoteData();

  useEffect(() => {
    delay(400).then(() => {
      const vs = getVersionsForNote(noteId);
      setVersions(vs);
      const p: Record<string, boolean> = {};
      vs.forEach((v) => {
        if (v.isPinned) p[v.id] = true;
      });
      setPinned(p);
    });
  }, [noteId, getVersionsForNote]);
                ...
                <View key={v.id} style={styles.versionCard}>
                  <View style={styles.versionHeader}>
                    ...
                    <View style={styles.versionMeta}>
                      <Text style={styles.versionDate}>{fmtDate(v.savedAt)}</Text>
                      {user.role === "lecturer" && (
                        <TouchableOpacity
                          onPress={() => togglePin(v.id)}
                          style={styles.pinBtn}
                        >
                          <Ionicons
                  ...
                  <Text style={styles.versionAuthor}>Saved by {v.savedBy}</Text>
                  <TouchableOpacity
                    onPress={() => setViewVersion(v)}
        ...
        {viewVersion && (
          <>
            ...
            <Text style={styles.versionPreviewMeta}>
              Saved by {viewVersion.savedBy} · {fmtDate(viewVersion.savedAt)}
            </Text>
          </>
        )}
      </Modal>
